...
def Print_ServiceObjectGroup(obj):
    ...
    if obj.Type == 'service':
        for service in  obj.Service:
            ...
            if service.Protocol == 'icmp':
                print 'type icmp'
                print 'dst icmp type',service.ICMPType
            elif service.Protocol == 'ip':
                print 'type ip'
                print 'dst ip type',service.Protocol
                
            elif service.Protocol == 'tcp' or service.Protocol == 'udp' or service.Protocol == 'tcp-udp':
                
                print 'type tcp/udp'
                print 'src port ',service.SrcPort.Name
                print 'dst port ',service.DstPort.Name
    else:
        print obj.Service[0].Protocol
        for service in  obj.Service:
            if service.Protocol == 'tcp' or service.Protocol == 'udp' or service.Protocol == 'tcp-udp':
                
                print 'type tcp/udp'
                print service.Name
           
def Print_ServiceObject(obj):
    if obj.Protocol == 'icmp':
        print 'type icmp'
        print 'dst icmp type',obj.ServiceObject.ICMPType
    elif obj.Protocol == 'ip':
        print 'dst ip type',obj.ServiceObject.Protocol
    elif obj.Protocol == 'tcp' or obj.Protocol == 'udp' :
        print 'src port ',obj.ServiceObject.SrcPort.port
        print 'dst port ',obj.ServiceObject.DstPort.port

def Print_CiscoPort(obj):
    print '     ',obj.Name
    
for acl in R.ACLs_Object_List:
    
    
    print 'Name',acl.Name
    
    print 'LineNo',acl.LineNo
    print 'Type',acl.Type
    print 'Action',acl.Action
    print 'Protocol',acl.Protocol

    if type(acl.Protocol).__name__ == 'ServiceObject':
        
        if acl.Protocol.Protocol == 'icmp':
            print 'dst icmp type',acl.Protocol.ServiceObject.ICMPType
        elif acl.Protocol.Protocol == 'ip':
            print 'dst ip type',acl.Protocol.ServiceObject.Protocol
        elif acl.Protocol.Protocol == 'tcp' or acl.Protocol.Protocol == 'udp' :
            print 'src port ',acl.Protocol.ServiceObject.SrcPort.port
            print 'dst port ',acl.Protocol.ServiceObject.DstPort.port

    elif  type(acl.Protocol).__name__ == 'ServiceObjectGroup':
        Print_ServiceObjectGroup(acl.Protocol)

    
    elif type(acl.Protocol).__name__ == 'ProtocolObjectGroup':
        for protocol in acl.Protocol.Protocol:
            print 'protocol list',protocol

            
        if type(acl.SrcPort).__name__ == 'ServiceObjectGroup':
            Print_ServiceObjectGroup(acl.SrcPort)
        elif type(acl.SrcPort).__name__ == 'CiscoPort':
            print 'src port'
            Print_CiscoPort(acl.SrcPort)
        else:
            print 'src port',acl.SrcPort

            
        if type(acl.DstPort).__name__ == 'ServiceObjectGroup':
            Print_ServiceObjectGroup(acl.DstPort)
        elif type(acl.DstPort).__name__ == 'CiscoPort':
            print 'dst port'
            Print_CiscoPort(acl.DstPort)
        else:
            print 'dst port',acl.DstPort            
            
    elif type(acl.Protocol).__name__ == 'str':
        if type(acl.SrcPort).__name__ == 'ServiceObjectGroup':
            Print_ServiceObjectGroup(acl.SrcPort)
        elif type(acl.SrcPort).__name__ == 'CiscoPort':
            print 'src port'
        ...
        if type(acl.DstPort).__name__ == 'ServiceObjectGroup':
            ...
            print 'dst port',acl.DstPort
        if acl.DstICMPType:
            print 'icmp type',acl.DstICMPType
            
    if type(acl.SrcNet).__name__ == 'NetworkObjectGroup':
        print 'SrcNet',acl.SrcNet.Network
    else:
        print 'SrcNet',acl.SrcNet

    
    if type(acl.DstNet).__name__ == 'NetworkObjectGroup':
        print 'DstNet',acl.DstNet.Network
    else:
        print 'DstNet',acl.DstNet

    print 'time',acl.TimeRange
    print 'inactive',acl.InActive
